...
A side-band analysis bases on assumption that a background kinematic shanges slowlly through a spectrum. Thanks to this kinematics of the background events from signal reagion can be well described by background outside the signal reagion. An example is show in the fig ??. The $p \pim \pip \pim$ spectrum after all the cuts and side-band substruction is presented in fig ???.
